  ...
  async function processarPDFs() {
    ...
    for (const arquivo of arquivos) {
      try {
        ...
        // ✅ ID ou IDR AUTOMÁTICO
        let match = texto.match(/(IDR?)\s+([A-Z0-9\-]+)/i);
        let tipoId = match ? match[1].toUpperCase() : "ID";
        let id = match ? match[2] : "SEM_ID";
        ...
        // ✅ NOME FINAL
        const novoNome = limparNome(
          `CTE ${cte} - ${tipoId} ${id} - ${motorista}.pdf`
        );
